...
def usr():
    ...
    tk.Label(userwindow, text='请管理员登录\n暨南大学\n图书馆', font=('Arial', 20)).place(x=110, y=40)
...
def lendbook_button():
    try:
        sql1 = 'begin;'
        sql2 = 'insert into borrow (s_id, s_name, b_id, b_name, borrow_date, return_date) values ("%s", "%s", "%s", "%s", curdate(),' \
...
def importbook_button():
    ...
    for i in range(nrows-1):
        ...
        try:
            ...
            tk.messagebox.showinfo(title='Hi', message='图书导入失败！')
        finally:
            cursor.close()
            conn.close()
# 编辑图书
